...
$edgefile = fopen('danishJaccardGraph.wadj', 'w+');
// Hopefully won't have to generate labels, since they're already in the danishHyperGraph.wadj.labels file
//$labelfile = fopen('danishJaccardGraph.wadj.labels', 'w+');
...
# Hash to store half of the matrix to avoid duplicating computations for the other half.
# Has the format [story_id1][story_id2]
$jaccard_similarities = array();
$all_jaccard_similarities = array();
...
echo "Mean Jaccard similarity: " . average($all_jaccard_similarities) . "\n";
echo "Median Jaccard similarity: " . median($all_jaccard_similarities) . "\n";

function ComputeJaccard($story1, $story2) {

    global $all_jaccard_similarities, $jaccard_similarities, $keyword_ids, $place_ids, $etk_index_ids, $tango_index_ids, $story_to_keywords, $story_to_places_mentioned, $story_to_etk_indices, $story_to_tango_indices, $keyword_gw, $place_gw, $etk_gw, $tango_gw;

    if (array_key_exists($story2, $jaccard_similarities) && array_key_exists($story1, $jaccard_similarities[$story2]))
        return $jaccard_similarities[$story2][$story1];
    ...
    $jaccardSimilarity = $jaccardNumerator / $jaccardDenominator;
    $jaccard_similarities[$story1][$story2] = $jaccardSimilarity;
    if ($jaccardSimilarity != 1) {
        $all_jaccard_similarities[] = $jaccardSimilarity;
    }
