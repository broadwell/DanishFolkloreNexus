...
$edgefile = fopen('danishHyperGraph.wadj', 'w+');
$labelfile = fopen('danishHyperGraph.wadj.labels', 'w+');
...
for ($rowNumber=1; $rowNumber<(sizeof($cosineMatrix)-1); $rowNumber++) {
    ...
    for ($colNumber=0; $colNumber<sizeof($columns); $colNumber++) {
	    ...
	    if (!array_key_exists($story_id, $story_info)) {
                echo 'INVALID STORY for story->story: ' . $story_id . "\n";
                continue;
            }
            ...
            if (in_array($relatedStoryID, $edges[$storyID]))
                echo "DUPLICATE EDGE: " . $storyID . "->" . $relatedStoryID ."\n";
            else
                $edges[$storyID][] = $relatedStoryID;


            if (array_key_exists($storyID . "," . $relatedStoryID, $edgeweights))
                echo "DUPLICATE EDGEWEIGHT: " . $storyID . ", " . $relatedStoryID . "\n";
            else
...
echo "Base Tango node is " . $max_story_id . "\n";
		...
		if (!array_key_exists($story_id, $edges)) {
			...
			if (array_key_exists($story_id, $story_info))
				$edges[$story_id] = array($tango_index_node);
                            ...
                            echo 'INVALID STORY for story->tango index: ' . $story_id . "\n";
                            continue;
                        }
		} else {
                        if (in_array($tango_index_node, $edges[$story_id]))
                            echo "DUPLICATE EDGE: " . $story_id . "->" . $tango_index_node ."\n";
                        else
			    $edges[$story_id][] = $tango_index_node;
		}

		if (array_key_exists($tango_index_node, $edges))
                    if (in_array($story_id, $edges[$tango_index_node]))
                        echo "DUPLICATE EDGE: " . $tango_index_node . "->" . $story_id ."\n";
                    else
...
echo "Base keyword node is " . $max_tango_node . "\n";
		...
		if (!array_key_exists($story_id, $edges)) {
			...
			if (array_key_exists($story_id, $story_info))
				$edges[$story_id] = array($keyword_node);
                            ...
                            echo 'INVALID STORY for story->keyword: ' . $story_id . "\n";
                            continue;
                        }
		} else {
                    if (in_array($keyword_node, $edges[$story_id]))
                        echo "DUPLICATE EDGE: " . $story_id . "->" . $keyword_node ."\n";
                    else
			$edges[$story_id][] = $keyword_node;
		}

		if (array_key_exists($keyword_node, $edges))
                    if (in_array($story_id, $edges[$keyword_node]))
                        echo "DUPLICATE EDGE: " . $keyword_node . "->" . $story_id ."\n";
                    else
	...
	if (!array_key_exists($story_id, $edges)) {
		...
		if (array_key_exists($story_id, $story_info))
			$edges[$story_id] = array($etk_index_node);
                    ...
                    echo 'INVALID STORY for story->ETK index: ' . $story_id . "\n";
                    continue;
                }
	} else {
            if (in_array($etk_index_node, $edges[$story_id]))
                echo "DUPLICATE EDGE: " . $story_id . "->" . $etk_index_node ."\n";
            else
		$edges[$story_id][] = $etk_index_node;
	}

	if (array_key_exists($etk_index_node, $edges))
            if (in_array($story_id, $edges[$etk_index_node]))
                echo "DUPLICATE EDGE: " . $etk_index_node . "->" . $story_id ."\n";
            else
		...
		if (!array_key_exists($story_id, $edges)) {
			...
			if (array_key_exists($story_id, $story_info))
				$edges[$story_id] = array($place_node);
                            ...
                            echo 'INVALID STORY for story->place mentioned: ' . $story_id . "\n";
                            continue;
                        }
		} else {
                    if (in_array($place_node, $edges[$story_id]))
                        echo "DUPLICATE EDGE: " . $story_id . "->" . $place_node ."\n";
                    else
			$edges[$story_id][] = $place_node;
		}
#		echo "Added edge between story $story_id and place mentioned $place_node\n";
		...
		if (!array_key_exists($place_node, $edges)) {
			$edges[$place_node] = array($story_id);
                    ...
                    if (in_array($story_id, $edges[$place_node]))
                        echo "DUPLICATE EDGE: " . $place_node . "->" . $story_id ."\n";
                    else
		        $edges[$place_node][] = $story_id;
		}
...
# Now write ALL of the observed edges to the edgelist file
