package flexlib.scheduling.scheduleClasses
{
   ...
   [Event(name="scheduleParametersChanged", type="flash.events.Event")]
   ...
   public class ScheduleNavigator extends EventDispatcher
   {
	  ...
	  public function onScroll( event : ScrollEvent ) : void
		...
		{
			// PMB here is where we make the change to zoom centered instead of left-justified
			/* What we know: the current content width (in pixels)
							 the current scroll position (also in pixels, presumably between 0 and old_contentWidth)
			   What we are given: the new content width
			   What else we can change: the scroll position
			   
			   NOTE: If we want truly centered zooming, we may have to change the startDate (and maybe also endDate)
			   		 values to indicate when the old values are scrolled off the screen. Otherwise the leftmost endpoint
			   		 will ALWAYS be the original startDate.
			   		 Have to change entryLayoutImpl.startDate to be specific.
			   		 HOW TO DO THIS
			   	Assuming no scrolling:
			   		 If the new contentWidth is bigger than the old one,
			   		 	half the new width will be added to before the start point, half after.
			   		 	Need to find out how much TIME this number of pixels equals.
			*/
			...
			//Alert.show("new startDate is " + owner.startDate.toDateString() + ", endDate is " + owner.endDate.toDateString())	
			//Alert.show("new contentWidth is " + value + ", newStartDate is " + newStartDate.toString() + ", newEndDate is " + newEndDate.toString() + ", morePixels is " + morePixels + ", moreMilliseconds is " + moreMilliseconds);
			//dispatchEvent( new Event( "scheduleParametersChanged", true, false) );
